...
# Iterando sobre as amostras
for coluna in coluna_amostral:
    media_amostrais = [np.mean(amostra) for amostra in amostras[coluna][5]]
    variancia_amostrais = [np.var(amostra) for amostra in amostras[coluna][5]]
    desvio_amostrais = [np.std(amostra) for amostra in amostras[coluna][5]]
    plt.plot([coluna]*n_amostral, media_amostrais, 'b.', alpha=0.6)
    plt.plot([coluna]*n_amostral, variancia_amostrais, 'g.', alpha=0.6)
    plt.plot([coluna]*n_amostral, desvio_amostrais, 'r.', alpha=0.6)

# Configure os rótulos do eixo x
plt.xticks(rotation=45)

# Configure os rótulos do eixo y
plt.ylabel('Valores')
plt.title('Média das Médias, Variância das Variâncias e Desvio Padrão dos Desvios (Tamanho da Amostra 5)')
plt.legend(['Média das Médias', 'Variância das Variâncias', 'Desvio Padrão dos Desvios'])
